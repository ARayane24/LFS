...
export OP_Glibc="glibc-2.39"               # contains the main C library. This library provides the basic routines for allocating memory, searching directories, opening and closing files, reading and writing files, string handling, pattern matching, arithmetic, and so on.
...
export OR_M4="m4-1.4.19"                   # contains a macro processor.
export OP_BinutilsOP_Bc="bc-6.7.5"         # contains an arbitrary precision numeric processing language.
export OP_Flex="flex-2.6.4"                # contains a utility for generating programs that recognize patterns in text.
export OP_Tcl="tcl-8.6.13"                 # contains the Tool Command Language, a robust general-purpose scripting language. The Expect package is written in Tcl (pronounced "tickle")
export OP_Expect="expect-5.45.4"           # contains tools for automating, via scripted dialogues, interactive applications such as telnet, ftp, passwd, fsck, rlogin, and tip. Expect is also useful for testing these same applications as well as easing all sorts of tasks that are prohibitively difficult with anything else. The DejaGnu framework is written in Expect.
...
export OP_Sed="sed-4.9"                    # contains stream editor.
...
export OP_Less="Less-643"                  # contains a text file viewer.
...
export OP_Libelf="elfutils-0.190"          # a library for handling ELF (Executable and Linkable Format) files.
...
export OP_MarkupSafe="markupsafe-2.1.5"    #
export OP_Jinja="jinja2-3.1.3"             #
export OP_Man_DB="man_db-2.12.0"           #
export OP_Procps_ng="procps-ng-4.0.4"      #
export OP_Texinfo="texinfo-7.1"            #
export OP_Util_linux="util-linux-2.39.3"   #
export OP_E2fsprogs="e2fsprogs-1.47.0"     #
export OP_Sysklogd="sysklogd-1.5.1"        #
export OP_Sysvinit="sysvinit-3.08"         #
