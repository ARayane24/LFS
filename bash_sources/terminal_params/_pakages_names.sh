...
export Ncurses_Tool="ncurses-6.4-20230520"   # contains libraries for terminal-independent handling of character screens.
export Bash_Tool="bash-5.2.21"               # contains the Bourne-Again Shell
export Coreutils_Tool="coreutils-9.4"        # contains the basic utility programs needed by every operating system.
...
export Findutils_Tool="findutils-4.9.0"      # programs to find files. Programs are provided to search through all the files in a directory tree and to create, maintain, and search a database (often faster than the recursive find, but unreliable unless the database has been updated recently). Findutils also supplies the xargs program, which can be used to run a specified command on each file selected by a search.
...
export Xz_Tool="xz-5.4.6"                    # contains programs for compressing and decompressing files
...
export Gettext_Tool="gettext-0.22.4"         # contains utilities for internationalization and localization. These allow programs to be compiled with NLS (Native Language Support), enabling them to output messages in the user's native language.
export Bison_Tool="bison-3.8.2"              # contains a parser generator.
export Perl_Tool="perl-5.38.2"               # contains the Practical Extraction and Report Language.
export Python_Tool="Python-3.12.2"           # contains the Python development environment
...
export OP_Bc="bc-6.7.5"                    # contains an arbitrary precision numeric processing language.(NOTE:: use?)
export OP_Flex="flex-2.6.4"                # ***contains a utility for generating programs that recognize patterns in text.
export OP_Tcl="tcl8.6.13"                  # **contains the Tool Command Language, a robust general-purpose scripting language. The Expect package is written in Tcl (pronounced "tickle")
